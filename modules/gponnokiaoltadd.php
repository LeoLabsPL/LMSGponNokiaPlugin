...
if (isset($_POST['netdev'])) {
    ...
    if ($netdevdata['ports'] == '') {
        ...
        $netdevdata['ports'] = $netdevdata['ports'];
    }
    ...
    if (intval($netdevdata['snmp_version'])>2) {
        ...
        if (strlen(trim($netdevdata['snmp_privacy_passphrase']))==0) {
            $error['snmp_privacy_passphrase'] = trans('Provide a pass passage');
        }
    ...
    if (!$error) {
        ...
        $SESSION->redirect('?m=gponnokiaoltinfo&id=' . $netdevid);
    }
...
$SMARTY->display('gponnokiaoltadd.html');
