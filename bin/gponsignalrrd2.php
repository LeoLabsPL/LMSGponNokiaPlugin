#!/usr/bin/env php
<?php
// używać gdyby pierwszy skrypt sprawiał trudności bądź nie wykonywał się do końca
...
if (!empty($olts)) {
    // Przetwarzanie sekwencyjne - po kolei dla każdego OLT
    foreach ($olts as $olt) {
        echo "Processing OLT: " . $olt['name'] . " (ID: " . $olt['id'] . ")" . PHP_EOL;
        print_r($olt);
        ...
        if (empty($olt_ports) || !is_array($olt_ports)) {
            echo "No ports found for OLT: " . $olt['name'] . PHP_EOL;
            continue;
        }
        ...
        echo "Finished processing OLT: " . $olt['name'] . PHP_EOL . PHP_EOL;
    }
}
